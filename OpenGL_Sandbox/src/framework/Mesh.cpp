...
Mesh::Mesh(const std::vector<Vector2D> &vertices, const std::vector<unsigned int> &indices,
...
Mesh::Mesh(const std::vector<Vector3D> &vertices, const std::vector<unsigned int> &indices,
        const std::vector<Vector2D> &uvs, std::shared_ptr<Material> &material) {
...
void Mesh::SetMesh(const std::vector<Vector2D> &vertices, const std::vector<unsigned int> &indices, const std::vector<Vector2D> &uvs) {
    ...
    for (int i = 0; i < vertices.size(); ++i) {
        ...
        if (textured) {
            rawVerts.push_back(uvs[i].u());
            rawVerts.push_back(uvs[i].v());
        }
    }
...
void Mesh::SetMesh(const std::vector<Vector3D> &vertices, const std::vector<unsigned int> &indices, const std::vector<Vector2D> &uvs) {
    bool textured = m_material->GetRenderMode() == RenderMode::TEXTURED;
    m_vertexArray = new VertexArray();
    std::vector<float> a;
    std::vector<float> b;
